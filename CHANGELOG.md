...
- Check for plugin updates from the Plugins page. AxeAI also checks every six hours.
...
- Split the thread panel into several tabs. AxeAI restores the layout later.
- Use dark mode on the AxeAI website.

### Built-in plugin updates

- **File Editor.** Enable it from Extensions > Installed Plugins. Open, edit, and save text files inside AxeAI.
...
All built-in agent providers now use the provider API. You can use the same API to build your own provider with a first-class AxeAI timeline.

### CLI

- Use `--plan` with `bb thread tell` or `bb thread spawn` to enter Plan mode.
- Use `bb thread log --all` to read the full thread history.
- Move a local plugin with `bb plugin install path:<new directory>`. AxeAI keeps its data and settings.
...
- A thread now holds a new message while it waits for your answer. AxeAI delivers the message after your answer.
...
- The `bb connect` tunnel renews active sessions and retries rejected connections.
